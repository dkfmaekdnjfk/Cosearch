...
> ⚠️ **노트 작성 전 원문 추출 필수 (이 순서를 지킬 것)**
>
> HTML 파일을 Grep 또는 Read로 직접 읽어 아래 3항목을 먼저 뽑아낸다.
> 추출 없이 DR 요약만 보고 쓰는 것은 금지.
>
> 1. **연구 대상 및 조건** — 피험자/샘플, 실험 조건, N값 등 (Methods 또는 Results에서)
> 2. **핵심 방법** — 주요 분석/실험 Methods 단락에서 1~2문장 직접 인용
> 3. **주요 결과 수치** — 핵심 효과 크기, 비교값, 통계 수치 등 논문 고유 수치
...
본문: **위에서 추출한 원문 내용만** 작성. 반드시 Methods 직접 인용구(`"..."`) 1개 이상 포함.
