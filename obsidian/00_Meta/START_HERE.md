...
> **CLAUDE.md는 80줄 이하로 유지한다.** 매 세션 시스템 프롬프트로 통째로 로드되므로 길수록 토큰 비용이 늘고 Claude의 주의가 분산된다. 상세 절차나 규칙은 `CONVENTIONS.md` 또는 스킬 파일에 두고, CLAUDE.md에는 "어디를 보라"는 포인터만 남긴다.

## 2. 코드 구조 세팅

> ⚠️ **기존 코드가 있다면 반드시 먼저 백업할 것.**
> Claude는 gitignore 검토가 완전하지 않으며, 파일 이동 중 실수로 코드를 삭제한 사례가 있다.
> 재구조화 전에 반드시 커밋하거나 별도 복사본을 만들어두고, Claude가 요청할 때 성의 없이 허락하지 말 것.

### 코드 재구조화 안전 절차

Claude에게 파일 이동·폴더 재구조화를 요청하기 전 이 순서를 따른다:

1. **로컬 커밋 먼저** — 현재 상태를 커밋한다. 커밋되지 않은 변경이 있으면 Claude가 작업을 시작하지 않아야 한다.
2. **`.gitignore` 검토** — 이동 대상 경로가 gitignore에 걸리는지 확인한다. **Claude의 gitignore 검토는 완전하지 않다.** 이동 후 `git status`로 추적 여부를 반드시 재확인한다.
3. **백업** — 복사본 또는 별도 브랜치를 만들어둔다. 백업 없이 진행하면 복구가 불가능할 수 있다.
4. **명확한 허락** — "ㅇㅇ", "그래" 같은 성의 없는 답변으로 허락하지 말 것. Claude는 재확인을 요청해야 하며, 명확한 확인 후에만 실행해야 한다.

`code/` 폴더를 아래 규칙에 맞게 정리한다:

| 종류 | 위치 | 명명 | 예시 |
|------|------|------|------|
| 실행 스크립트 (.py) | `code/scripts/` | `topic_YYMMDD.py` | `run_pca_260515.py` |
...
| 기존 코드 | `code/legacy/` | 그대로 유지 | — |

**날짜는 topic 뒤에 붙인다.** 파일은 주제로 검색하고, 날짜는 버전 식별용. EXP 노트 날짜와 스크립트 날짜가 대응되어야 한다 (`EXP 260515 ...` ↔ `*_260515.py`).

패키지를 `code/` 아래 두었다면 import 경로 주의:
```bash
